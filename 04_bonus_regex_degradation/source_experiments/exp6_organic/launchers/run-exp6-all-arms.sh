...
STATUS_DIR="${STATUS_DIR:-/tmp/exp6-status}"
mkdir -p "$STATUS_DIR"
...
run_one() {
  ...
  ARM="$arm" bash scripts/fleet-1p7b-exp6-organic.sh 2>&1 | tee "$log"
...
echo ""
echo "########################################"
echo "# Exp 6: 3 arms sequential              #"
echo "########################################"

# Order: primary Michael-scenario arm first.
run_one "exp6-regex_fix"    "regex_fix"
run_one "exp6-clamp_fix"    "clamp_fix"
run_one "exp6-math_control" "math_control"
...
{
  echo "# Exp 6 (organic paste) — run summary"
  ...
  echo ""
  echo "## S3"
  echo "s3://skyrl-checkpoints/fleet-side-effects-bugs-1p7b/exp6-{regex_fix,clamp_fix,math_control}/"
} > "$SUMMARY"
cat "$SUMMARY"
aws s3 cp --quiet "$SUMMARY" "s3://skyrl-checkpoints/fleet-side-effects-bugs-1p7b/SUMMARY.md" 2>/dev/null || true
