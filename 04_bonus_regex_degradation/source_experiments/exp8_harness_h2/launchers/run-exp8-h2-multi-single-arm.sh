#!/usr/bin/env bash
# Single-arm orchestrator for exp8 (harness H2 MULTI-TURN).
# Mirrors run-exp8-h2-single-arm.sh but points at the multi-turn launcher.
#
...
STATUS_DIR="${STATUS_DIR:-/tmp/exp8-multi-status}"
mkdir -p "$STATUS_DIR"
...
# Pre-flight: fabricmanager + nvidia-smi (copied from single-turn orchestrator).
...
ARM="$ARM" bash scripts/fleet-1p7b-exp8-h2-multi.sh 2>&1 | tee "$log"
rc=${PIPESTATUS[0]}
